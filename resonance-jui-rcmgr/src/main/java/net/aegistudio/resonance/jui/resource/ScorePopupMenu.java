...
import net.aegistudio.resonance.jui.util.Overwrite;
...
public class ScorePopupMenu extends JPopupMenu {
	...
	public ScorePopupMenu(Theme theme, ResourceModel resModel, ScoreEntry scoreEntry){
		this.scoreEntry = scoreEntry;
		
		if(this.scoreEntry == null)
			score = new JMenuItem("Score");
		else score = new JMenuItem(scoreEntry.score.name());
		...
		theme.configure(createScore, "resource.score");
		...
		theme.configure(eraseScore, "common.erase");
		eraseScore.addActionListener(new ActionListener(){
			@Override
			public void actionPerformed(ActionEvent arg0) {
				int answer = JOptionPane.showConfirmDialog(scoreEntry,
						String.format("Are you sure to erase the score %s (Can't be undo)?", scoreEntry.score.name()),
						"Erasure confirm", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
				if(answer == JOptionPane.YES_OPTION) try {
					resModel.removeScore(scoreEntry);
				}
				catch(Exception e) {
					JOptionPane.showConfirmDialog(scoreEntry, e.getMessage(), 
							"Error while erasing", JOptionPane.YES_OPTION, JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		exportScore.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				midiChooser.setFileFilter(midiFilter);
				midiChooser.setSelectedFile(new File(scoreEntry.score.name().concat(".mid")));
